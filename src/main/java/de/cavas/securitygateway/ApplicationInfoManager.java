...
//            this.instanceInfo = new EurekaConfigBasedInstanceInfoProvider(config).get();
     ...
     */
    public synchronized void setInstanceStatus(de.cavas.securitygateway.InstanceInfo.InstanceStatus status) {
        de.cavas.securitygateway.InstanceInfo.InstanceStatus prev = instanceInfo.setStatus(status); // setStatus(status);
        if (prev != null) {
